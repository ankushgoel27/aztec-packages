...
AvmFlavor::AllConstRefValues::AllConstRefValues(
    ...
    , binary_start(il[81])
    , bytecode_end_latch(il[82])
    , bytecode_length_remaining(il[83])
    , bytecode_packed(il[84])
    , bytecode_running_hash(il[85])
    , cmp_a_hi(il[86])
    , cmp_a_lo(il[87])
    , cmp_b_hi(il[88])
    , cmp_b_lo(il[89])
    , cmp_borrow(il[90])
    , cmp_clk(il[91])
    , cmp_cmp_rng_ctr(il[92])
    , cmp_input_a(il[93])
    , cmp_input_b(il[94])
    , cmp_op_eq(il[95])
    , cmp_op_eq_diff_inv(il[96])
    , cmp_op_gt(il[97])
    , cmp_p_a_borrow(il[98])
    , cmp_p_b_borrow(il[99])
    , cmp_p_sub_a_hi(il[100])
    , cmp_p_sub_a_lo(il[101])
    , cmp_p_sub_b_hi(il[102])
    , cmp_p_sub_b_lo(il[103])
    , cmp_range_chk_clk(il[104])
    , cmp_res_hi(il[105])
    , cmp_res_lo(il[106])
    , cmp_result(il[107])
    , cmp_sel_cmp(il[108])
    , cmp_sel_rng_chk(il[109])
    , cmp_shift_sel(il[110])
    ...
    , keccakf1600_clk(il[116])
    , keccakf1600_input(il[117])
    , keccakf1600_output(il[118])
    , keccakf1600_sel_keccakf1600(il[119])
    , main_abs_da_rem_gas(il[120])
    , main_abs_l2_rem_gas(il[121])
    , main_alu_in_tag(il[122])
    , main_base_da_gas_op_cost(il[123])
    , main_base_l2_gas_op_cost(il[124])
    , main_bin_op_id(il[125])
    , main_call_ptr(il[126])
    , main_da_gas_remaining(il[127])
    , main_da_out_of_gas(il[128])
    , main_dyn_da_gas_op_cost(il[129])
    , main_dyn_gas_multiplier(il[130])
    , main_dyn_l2_gas_op_cost(il[131])
    , main_emit_l2_to_l1_msg_write_offset(il[132])
    , main_emit_note_hash_write_offset(il[133])
    , main_emit_nullifier_write_offset(il[134])
    , main_emit_unencrypted_log_write_offset(il[135])
    , main_ia(il[136])
    , main_ib(il[137])
    , main_ic(il[138])
    , main_id(il[139])
    , main_id_zero(il[140])
    , main_ind_addr_a(il[141])
    , main_ind_addr_b(il[142])
    , main_ind_addr_c(il[143])
    , main_ind_addr_d(il[144])
    , main_internal_return_ptr(il[145])
    , main_inv(il[146])
